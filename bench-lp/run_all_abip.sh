...
if [ "$#" -ne 5 ]; then
  ...
  echo "Usage: <dataset directory> <output_directory> <time limit for each instance> <precision> <direct:0 / indirect:1> <func>" 1>&2
  exit -1
fi
...
for f in $(/bin/ls $input); do
  ...
  addpath matlab; addpath 'bench-lp'; test_one_abip('$input/$f', '$abipname', 1e-$precision, $timelimit); exit;"
  full_cmd="nohup timeout $timelimit \
    $MATLAB_HOME/bin/matlab -nodesktop -nodisplay -nosplash -noFigureWindows -r \
    \"${mat_cmd}\" &> $output/${abipsrc}_${usedirect}_1e-$precision/$f.log "
  echo $full_cmd
done
