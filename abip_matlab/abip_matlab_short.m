function [x,y,s,info] = abip_matlab_short(data,params)
%------------------------------------------------------------------------
...
%% parameter setting 
if nargin==2
    if isfield(params,'max_outiters');   max_outiters = params.max_outiters;   end
...
for i=0:max_outiters-1   
    for j=0:max_iters-1
        ...
        %% solve linear system
        [ut, ~] = project_lin_sys(work, data, n, m, k, u, v, rho_y, use_indirect, ...
...
%                i, mu, k, err_pri, err_dual, gap, ttime);
    ...
    elseif error_ratio>1 && error_ratio<=3 && ratio > 0.1
        ...
        gamma = gamma*0.4;
        final_check = 1;
    elseif error_ratio>1 && error_ratio<=3 && ratio < 0.1 && double_check == 0
        sigma = 1;
        gamma = gamma*0.1;
        final_check = 1;
        double_check = 1; 
    elseif error_ratio>1 && error_ratio<=3 && ratio < 0.1 && double_check == 1
        sigma = 0.88;
        gamma = gamma*0.4;
        final_check = 1;
        double_check = 0;
...
%     v(m+1:l) = sigma.*v(m+1:l);
%     u(m+1:l) = sqrt(sigma).*u(m+1:l);
%     v(m+1:l) = sqrt(sigma).*v(m+1:l);
    ind1 = find(u(m+1:l) < v(m+1:l));
    ind2 = find(u(m+1:l) >= v(m+1:l));
    u(ind1) = sigma.*u(ind1);
    v(ind2) = sigma.*v(ind2); 
