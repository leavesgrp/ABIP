...
abip_int init_lasso(lasso **self, ABIPData *d, ABIPSettings *stgs){
    ...
    *self = this_lasso;
    ...
    this_lasso->rho_dr = (abip_float *)abip_malloc((m + n + 1) * sizeof(abip_float));
    ...
    if(this_lasso->sparsity){
        ...
        if(d->m<d->n) this_lasso->sc = 4;
        else this_lasso->sc = 1;
        this_lasso->sc_c = 1/d->lambda;
    ...
    abip_float *data_b = (abip_float*)abip_malloc(this_lasso->p*sizeof(abip_float));
    data_b[0] = 1;
    memcpy(&data_b[1],d->b,d->m*sizeof(abip_float));
    this_lasso->data->b = data_b;

    abip_float *data_c = (abip_float*)abip_malloc(this_lasso->q*sizeof(abip_float));
    data_c[0] = 0;
    data_c[1] = 2;
...
void scaling_lasso_data(lasso *self, ABIPCone *k){
    ...
    for (int i = 0 ; i < m; i++){
        self->D_hat[i] = self->D[i]*self->D[i]*self->sc_cone2 + self->stgs->rho_y;
    } 
...
abip_float lasso_inner_conv_check(lasso *self, ABIPWork *w){
    ...
    abip_float error_inner = ABIP(norm)(tem, m+n+1) / (1 + ABIP(norm)(w->u, m+n+1) + ABIP(norm)(w->v_origin, m+n+1));
...
void calc_lasso_residuals(lasso *self, ABIPWork *w, ABIPResiduals *r, abip_int ipm_iter, abip_int admm_iter){
    ...
    this_gap = ABS(0.5*ABIP(dot)(x,x,m)+ABIP(dot)(lambda_ones,beta_plus,n)+ABIP(dot)(lambda_ones,beta_minus,n)+0.5*ABIP(dot)(z,z,m)-ABIP(dot)(&self->data->b[1],z,m))/(1+ABS(0.5*ABIP(dot)(x,x,m)+ABIP(dot)(lambda_ones,beta_plus,n)+ABIP(dot)(lambda_ones,beta_minus,n)));
    
    r->last_ipm_iter = ipm_iter;
    r->last_admm_iter = admm_iter;

    r->dobj = (-0.5*ABIP(dot)(z,z,m) + ABIP(dot)(&self->data->b[1],z,m));
    r->pobj = (0.5*ABIP(dot)(x,x,m)+ABIP(dot)(lambda_ones,beta_plus,n)+ABIP(dot)(lambda_ones,beta_minus,n));
...
void init_lasso_precon(lasso *self){
    ...
    for(int i=1;i<self->p;i++){
        M[i] = 1 / (self->stgs->rho_y + M[i] + self->sc_cone2 * POWF(self->D[i -1], 2));
    }
}


abip_float get_lasso_pcg_tol(abip_int k, abip_float error_ratio, abip_float norm_p){
