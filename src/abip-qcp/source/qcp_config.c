...
abip_int init_qcp(qcp **self, ABIPData *d, ABIPSettings *stgs){
    ...
    *self = this_qcp;
    ...
    this_qcp->rho_dr = (abip_float *)abip_malloc((m + n + 1) * sizeof(abip_float));
...
void scaling_qcp_data(qcp *self, ABIPCone *k){
    ...
    abip_float sc = SQRTF(SQRTF(ABIP(norm_sq)(self->c, n) + ABIP(norm_sq)(self->b, m)));
    ...
    for(i = 0; i < n; i++){
        ...
        if (Q == ABIP_NULL || Q->p[i] == Q->p[i + 1]) {
            ...
            for (j = Q->p[i]; j < Q->p[i + 1]; j++) {
                ...
                if (Q->i[j] > i) tem = 0;
                else if (Q->i[j] == i) tem = Q->x[j] + self->rho_dr[m + i];
                else tem = Q->x[j];
...
void init_qcp_precon(qcp *self){
    ...
    for (int i = 0; i < self->q; i++) {
        for (int j = self->A->p[i]; j < self->A->p[i + 1]; j++) {
            self->L->M[i] += self->A->x[j] * self->A->x[j] / self->rho_dr[self->A->i[j]];
        }
    }
...
abip_int solve_qcp_linsys(qcp *self, abip_float *b, abip_float *pcg_warm_start, abip_int iter, abip_float error_ratio){
    ...
    if(self->stgs->linsys_solver == 3){  //pcg
        ...
        if(iter == -1){
            ...
            cg_its = ABIP(solve_linsys)(self, &b[m], n, &pcg_warm_start[m], error_ratio);
        }
