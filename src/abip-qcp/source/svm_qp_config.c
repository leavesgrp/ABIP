...
void svmqp_A_times(svmqp *self, const abip_float *x, abip_float *y) {
  ...
  for (int i = 0; i < self->m; i++) {
    y[i] +=
        1 / self->D[i] * (x[self->n + 1 + i] - x[self->n + 1 + self->m + i]);
  }
}
...
void scaling_svmqp_data(svmqp *self, ABIPCone *k) {
  ...
  abip_float sc =
      SQRTF(SQRTF(ABIP(norm_sq)(self->c, self->q) + ABIP(norm_sq)(self->b, m)));
  ...
  for (int i = 0; i < m; i++) {
    self->F[i] =
        self->stgs->rho_y + 2 / self->stgs->rho_x / POWF(self->D[i], 2);
  }

  for (int i = 0; i < self->q; i++) {
    if (i < self->n)
      self->H[i] = self->stgs->rho_x + self->Q->x[i];
    else
      self->H[i] = self->stgs->rho_x;
  }
...
void init_svmqp_precon(svmqp *self) {
  ...
  for (i = 0; i < self->p; i++) {
    ...
    self->L->M[i] +=
        1 / (self->E[i] * self->E[i] * self->H[self->A->n + self->p + i]);
  }
...
formulation
*/
abip_float get_svmqp_pcg_tol(abip_int k, abip_float error_ratio,
                             abip_float norm_p) {
  if (k == -1) {
    return 1e-9 * norm_p;
  } else {
...
abip_int solve_svmqp_linsys(svmqp *self, abip_float *b,
  ...
  if (self->stgs->linsys_solver == 3) {  // pcg
    ...
    abip_int cg_its =
        ABIP(solve_linsys)(self, &b[m], n, &pcg_warm_start[m], pcg_tol);
    ...
    if (m > n + 1 && self->stgs->linsys_solver != 3) {
      ...
      abip_float *tmp1 =
          (abip_float *)abip_malloc((n + 1) * sizeof(abip_float));
      ...
      abip_int cg_its =
          ABIP(solve_linsys)(self, b2, m, pcg_warm_start, error_ratio);
      if (iter >= 0) {
        self->L->total_cg_iters += cg_its;
      }
    }
