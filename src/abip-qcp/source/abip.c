...
static const char *HEADER[] = {
    " ipm iter ", " admm iter ", "     mu ",  " pri res ", " dua res ",
    " rel gap ",  " pri obj ",   " dua obj ", " kap/tau ", " time (s)",
};
...
static abip_int abip_isnan(abip_float x) {
  DEBUG_FUNC
  RETURN(x == NAN || x != x);
}

static void free_work(ABIPWork *w) {
  DEBUG_FUNC
...
static void print_init_header(spe_problem *spe) {
  DEBUG_FUNC
  ...
  if (stgs->normalize) {
    abip_printf(
        "eps_p = %.2e, eps_d = %.2e, eps_g = %.2e, alpha = %.2f, max_ipm_iters "
        "= %i, max_admm_iters = %i, normalize = %i\n"
        "rho_y = %.2e\n",
        stgs->eps_p, stgs->eps_d, stgs->eps_g, stgs->alpha,
        (int)stgs->max_ipm_iters, (int)stgs->max_admm_iters,
...
static void populate_on_failure(abip_int m, abip_int n, ABIPSolution *sol,
                                ABIPInfo *info, abip_int status_val,
                                const char *msg) {
  DEBUG_FUNC
...
static abip_int failure(ABIPWork *w, abip_int m, abip_int n, ABIPSolution *sol,
                        ABIPInfo *info, abip_int stint, const char *msg,
                        const char *ststr) {
  DEBUG_FUNC
...
static abip_int projection(ABIPWork *w, abip_int iter, spe_problem *spe,
                           ABIPResiduals *r) {
  DEBUG_FUNC
  ...
  if (spe->Q != ABIP_NULL || spe->stgs->linsys_solver != 3) {
    ...
    abip_float *warm_start =
        (abip_float *)abip_malloc((m + n) * sizeof(abip_float));
    ...
    if (spe->stgs->linsys_solver == 3) {  // create warm start for pcg
      ...
      pcg_tol = 0.2 * MIN(pcg_tol, ABIP(norm_inf)(warm_start, n) /
                                       POWF((abip_float)iter + 1, 1.5));
    ...
    abip_float b =
        ABIP(dot)(w->r, mu, m + n) - 2 * ABIP(dot)(w->r, tem, m + n) - eta;
    ...
    abip_float coef =
        -(-ABIP(dot)(spe->b, w3, m) + ABIP(dot)(spe->c, &w3[m], n)) /
        (1 - ABIP(dot)(spe->b, w->r, m) + ABIP(dot)(spe->c, &w->r[m], n));

    memcpy(w->u_t, w3, (m + n) * sizeof(abip_float));
    ABIP(add_scaled_array)(w->u_t, w->r, m + n, coef);
    if (iter > 0) {
      w->u_t[m + n] =
          eta - ABIP(dot)(spe->b, w->u_t, m) + ABIP(dot)(spe->c, &w->u_t[m], n);
    } else {
      w->u_t[m + n] = 1;
    }
...
static void update_dual_vars(ABIPWork *w) {
  DEBUG_FUNC
...
static void solve_barrier_subproblem(ABIPWork *w, const ABIPCone *c,
                                     spe_problem *spe) {
  DEBUG_FUNC
  ...
  abip_float tau = (tmp[l - 1] + SQRTF(tmp[l - 1] * tmp[l - 1] +
                                       4 * lambda / spe->rho_dr[l - 1])) /
                   2;
  ...
  if (c->qsize && c->q) {
    for (i = 0; i < c->qsize; ++i) {
      ...
      if (c->q[i] == 1) {
        ABIP(positive_orthant_barrier_subproblem)
        (&w->u[m + count], &tmp[m + count], lambda / spe->rho_dr[m + count], 1);
      } else {
        ABIP(soc_barrier_subproblem)
        (&w->u[m + count], &tmp[m + count], lambda / spe->rho_dr[m + count],
         c->q[i]);
      }
      count += c->q[i];
    }
  ...
  /*free cone*/
  if (c->f) {
    ABIP(free_barrier_subproblem)
    (&w->u[m + count], &tmp[m + count], lambda / spe->rho_dr[m + count], c->f);
    count += c->f;
  }

  /*zero cone*/
  if (c->z) {
    ABIP(zero_barrier_subproblem)
    (&w->u[m + count], &tmp[m + count], lambda / spe->rho_dr[m + count], c->z);
    count += c->z;
  }
  ...
  if (c->l) {
    ABIP(positive_orthant_barrier_subproblem)
    (&w->u[m + count], &tmp[m + count], lambda / spe->rho_dr[m + count], c->l);
    count += c->l;
  }
...
static abip_int indeterminate(ABIPWork *w, ABIPSolution *sol, ABIPInfo *info) {
  DEBUG_FUNC
...
static abip_int solved(ABIPWork *w, ABIPSolution *sol, ABIPInfo *info,
                       abip_float tau) {
  DEBUG_FUNC
...
static void sety(ABIPWork *w, ABIPSolution *sol) {
  DEBUG_FUNC
...
static void setx(ABIPWork *w, ABIPSolution *sol) {
  DEBUG_FUNC
...
static void sets(ABIPWork *w, ABIPSolution *sol) {
  DEBUG_FUNC
...
static abip_int infeasible(ABIPWork *w, ABIPSolution *sol, ABIPInfo *info,
                           abip_float bt_y) {
  DEBUG_FUNC
...
static abip_int unbounded(ABIPWork *w, ABIPSolution *sol, ABIPInfo *info,
                          abip_float ct_x) {
  DEBUG_FUNC
...
static abip_int is_solved_status(abip_int status) {
  RETURN status == ABIP_SOLVED || status == ABIP_SOLVED_INACCURATE;
}

static abip_int is_infeasible_status(abip_int status) {
  RETURN status == ABIP_INFEASIBLE || status == ABIP_INFEASIBLE_INACCURATE;
}

static abip_int is_unbounded_status(abip_int status) {
  RETURN status == ABIP_UNBOUNDED || status == ABIP_UNBOUNDED_INACCURATE;
}

static void get_info(ABIPWork *w, ABIPSolution *sol, ABIPInfo *info,
                     ABIPResiduals *r, abip_int ipm_iter, abip_int admm_iter) {
  DEBUG_FUNC
...
static void get_solution(ABIPWork *w, spe_problem *spe, ABIPSolution *sol,
                         ABIPInfo *info, ABIPResiduals *r, abip_int ipm_iter,
                         abip_int admm_iter) {
  DEBUG_FUNC
...
static void print_summary(ABIPWork *w, spe_problem *spe, abip_int i, abip_int j,
                          abip_int k, ABIPResiduals *r,
  ...
  abip_printf("|u - u_t| = %1.2e, ",
              ABIP(norm_diff)(w->u, w->u_t, w->n + w->m + 2));
...
static void print_header(ABIPWork *w) {
  DEBUG_FUNC
...
static void print_footer(const ABIPData *d, spe_problem *spe, ABIPSolution *sol,
                         ABIPWork *w, ABIPInfo *info, abip_int k) {
  DEBUG_FUNC
  ...
  if (is_infeasible_status(info->status_val)) {
    ...
    abip_printf(
        "primal res: |Ax - b|_inf / (1 + max(|Ax|_inf, |b|_inf)) = %.4e\n",
        info->res_pri);
    abip_printf(
        "dual res: |Qx - A'y + c - s|_inf / (1 + max(|Qx|_inf + |c|_inf)) = "
        "%.4e\n",
        info->res_dual);
    abip_printf(
        "rel gap: |x'Qx + c'x - b'y| / (1 + max(|x'Qx| + |c'x| + |b'y|)) = "
        "%.4e\n",
        info->rel_gap);
...
static abip_int has_converged(ABIPWork *w, spe_problem *spe, ABIPResiduals *r,
...
static abip_int validate(const ABIPData *d, const ABIPCone *k,
                         spe_problem *spe) {
  DEBUG_FUNC
  ...
  if (stgs->eps_p <= 0 || stgs->eps_d <= 0 || stgs->eps_g <= 0 ||
      stgs->eps_inf <= 0 || stgs->eps_unb <= 0) {
...
static ABIPWork *init_work(spe_problem *s, ABIPCone *k) {
  DEBUG_FUNC
...
static abip_int pre_calculate(ABIPWork *w, spe_problem *spe) {
  DEBUG_FUNC
...
abip_int update_work(const ABIPData *d, ABIPWork *w, const ABIPSolution *sol,
                     const ABIPCone *c, spe_problem *spe
...
abip_float adjust_barrier(ABIPWork *w, ABIPResiduals *r, spe_problem *spe) {
  ...
  abip_float ratio =
      w->mu / MIN(MIN(spe->stgs->eps_p, spe->stgs->eps_d), spe->stgs->eps_g);
  abip_float gamma;
...
abip_int ABIP(solve)(ABIPWork *w, const ABIPData *d, ABIPSolution *sol,
                     ABIPInfo *info, ABIPCone *c, spe_problem *s) {
  DEBUG_FUNC
  ...
  for (i = 0; i < s->stgs->max_ipm_iters; ++i) {
    for (j = 0; j < s->stgs->max_admm_iters; ++j) {
      ...
      if (projection(w, k, s, r) < 0) {
        RETURN failure(w, w->m, w->n, sol, info, ABIP_FAILED,
      ...
      if (abip_is_interrupted()) {
        RETURN failure(w, w->m, w->n, sol, info, ABIP_SIGINT, "Interrupted",
                       "Interrupted");
      }

#if EXTRA_VERBOSE > 0
      abip_printf("primal error: %.4f, dual error: %.4f, gap: %.4f\n",
                  r.err_pri / w->eps_p, r.err_dual / w->eps_d,
                  r.gap / w->eps_g);
#endif
      ...
      if ((j + 1) % s->stgs->inner_check_period == 0 || r->error_ratio <= 8) {
        ...
        if ((info->status_val = has_converged(w, s, r, i, k)) != 0 ||
            k + 1 >= s->stgs->max_admm_iters * s->stgs->max_ipm_iters ||
            i + 1 >= s->stgs->max_ipm_iters ||
            ABIP(tocq)(&solve_timer) >
                time_limit_left)  // max running time is time_limit s
        {
    ...
    if (s->sparsity || (i + 1) % s->stgs->outer_check_period == 0) {
      ...
      if ((info->status_val = has_converged(w, s, r, i, k)) != 0 ||
          k + 1 >= s->stgs->max_admm_iters * s->stgs->max_ipm_iters ||
          i + 1 >= s->stgs->max_ipm_iters ||
          ABIP(tocq)(&solve_timer) > time_limit_left) {
        if (s->stgs->verbose && k > 0) {
          printf("\nin last admm iter:\n");
          print_summary(w, s, i, j, k, r, &solve_timer);
          abip_printf("total admm iter is %i\n", k);
...
abip_int abip(const ABIPData *d, ABIPSolution *sol, ABIPInfo *info,
  ...
  if (d->stgs->prob_type == 0)
    prob_type = LASSO;
  else if (d->stgs->prob_type == 1)
    prob_type = SVM;
  else if (d->stgs->prob_type == 2)
    prob_type = QCP;
  else if (d->stgs->prob_type == 3)
    prob_type = SVMQP;

  ABIP(init_problem)(&s, d, d->stgs, prob_type);
  abip_int status;
  ABIPWork *w = ABIP(init)(d, info, s,
                           K);  // call init_work() to call init_lin_sys_work()
                                // to perform LDL' factorization
  ...
  if (w) {
    ...
    status = failure(ABIP_NULL, d ? d->m : -1, d ? d->n : -1, sol, info,
                     ABIP_FAILED, "could not initialize work", "Failure");
  }
