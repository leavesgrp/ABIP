...
abip_int init_svm(svm **self, ABIPData *d, ABIPSettings *stgs){
    ...
    *self = this_svm;
    ...
    abip_float *data_b = (abip_float*)abip_malloc(this_svm->p*sizeof(abip_float));
    ...
    abip_float *data_c = (abip_float*)abip_malloc(this_svm->q*sizeof(abip_float));
    ...
    if((d->m<10*d->n)&&(10*d->m>d->n)){
        ...
        if(d->lambda >= 1){
            this_svm->sc_c = MAX(0.2,POWF(0.2,(log(2*d->lambda)/log(10)))*7.5);
        ...
        if(d->n < 10){
            ...
            this_svm->sc_cone1 = 6;
            if(d->lambda < 0.002){
                this_svm->sc_cone2 = this_svm->sc_cone2 - 3*log(this_svm->lambda*500)/log(10);
            }
        }
        else if(d->lambda >= 1){
            this_svm->sc = 1;
    ...
    this_svm->sc_E = (abip_float*)abip_malloc((this_svm->n+1)*sizeof(abip_float));
    this_svm->sc_F = (abip_float*)abip_malloc((this_svm->n)*sizeof(abip_float));
    ...
    this_svm->wH = (abip_float*)abip_malloc((this_svm->n+1)*sizeof(abip_float));
...
void svm_A_times(svm *self, const abip_float *x, abip_float* y){
    ...
    for(int i=0;i<m;i++){
        y[i+1] += (self->wB[i] * x[i + 3*n+4] - self->wC[i] * x[i + 3*n + 4 + m]);
    }
...
abip_float svm_inner_conv_check(svm *self, ABIPWork *w)
{
    ...
    abip_float err_inner = ABIP(norm)(Pu_v,l) / (1 + SQRTF(ABIP(norm_sq)(w->u,l)+ ABIP(norm_sq)(w->v_origin, l)));
...
void scaling_svm_data(svm *self, ABIPCone *k){
    ...
    for(int i=0;i<m;i++){
        self->wF[i] = self->wB[i]*self->wB[i] + self->wC[i]*self->wC[i] + self->stgs->rho_y;
    }

    for(int i=0;i<n;i++){
        self->wG[i] = self->wD[i]*self->wD[i] + 2*self->wE[i]*self->wE[i] + self->stgs->rho_y;
    }
...
void calc_svm_residuals(svm *self, ABIPWork *w, ABIPResiduals *r, abip_int ipm_iter, abip_int admm_iter){
    ...
    abip_float b = (w->u[m+3*n+3] - w->u[m+4*n+4])/r->tau * self->sc_E[n]/self->sc_b;
    w1[n] = b;
...
void init_svm_precon(svm *self){
    ...
    for(int i=1;i<self->m+1;i++){
        M[i] = 1 / (self->stgs->rho_y + M[i] + POWF(self->wB[i-1], 2) + POWF(self->sc_D[i -1], 2));
    }

    for(int i=self->m + 1;i<self->m+self->n+1;i++){
        M[i] = 1 / (self->stgs->rho_y + 2 * POWF(self->wE[i - self->m + 1], 2) + POWF(self->wD[i - self->m + 1], 2));
    }
}

abip_float get_svm_pcg_tol(abip_int k, abip_float error_ratio, abip_float norm_p){
