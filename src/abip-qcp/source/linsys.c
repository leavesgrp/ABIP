...
char *ABIP(get_lin_sys_method)(spe_problem *spe) {
  ...
  if (spe->stgs->linsys_solver == 0) {
    ...
    sprintf(tmp, "sparse-direct using sparse cholesky, nnz in A = %li",
            (long)n);
...
char *ABIP(get_lin_sys_summary)(spe_problem *self, ABIPInfo *info) {
  ...
  info->avg_linsys_time =
      self->L->total_solve_time / (info->admm_iter + 1) / 1e3;

  if (self->stgs->linsys_solver == 3) {
    sprintf(str,
            "\tLin-sys: avg # CG iterations: %2.2f, avg solve time per admm "
            "iter: %1.2es\n",
            (abip_float)self->L->total_cg_iters / (info->admm_iter + 1),
            info->avg_linsys_time);
  } else {
    sprintf(str,
            "\tLin-sys: nnz in L factor: %li, avg solve time per admm iter: "
            "%1.2es\n",
...
abip_int ABIP(validate_lin_sys)(const ABIPMatrix *A) {
  ...
  if (((abip_float)Annz / A->m > A->n) || (Annz <= 0)) {
    abip_printf(
        "ERROR: the number of nonzeros in A = %li, outside of valid range!\n",
        (long)Annz);
    return -1;
  }
  ...
  if (r_max > A->m - 1) {
    abip_printf(
        "ERROR: the number of rows in A is inconsistent with input "
        "dimension!\n");
    return -1;
  }
...
static void print_A_matrix(const ABIPMatrix *A) {
  abip_int i;
  abip_int j;
...
*/
// y += A'*x
void ABIP(accum_by_Atrans)(const ABIPMatrix *A, const abip_float *x,
                           abip_float *y) {
  abip_int p;
  abip_int j;
...
static void _ldl_solve(abip_float *b, cs *L, abip_float *Dinv, abip_int *P,
                       abip_float *bp) {
...
abip_int init_pardiso(spe_problem *self) {
  MKL_INT PARDISO_PARAMS_LDL[64] = {
      ...
      1,
      /* Non-default value */ 3,
      /* P Nested dissection */ 0, /* Reserved          */
      0,
      /* No CG             */ 0,
      /* No user permutation */ 0, /* Overwriting    */
      0,
      /* Refinement report */ 0,
      /* Auto ItRef step     */ 0, /* Reserved          */
      8,
      /* Perturb           */ 0,
      /* Disable scaling     */ 0, /* No transpose      */
      0,
      /* Disable matching  */ 0,
      /* Report on pivots    */ 0, /* Output            */
      0,
      /* Output            */ 0,
      /* Output              */ -1, /* No report         */
      0,
      ...
      0,
      /* nPosEigVals       */ 0,
      /* nNegEigVals         */ 0, /* Classic factorize */
      0,
      0,
      0, /* Matrix checker    */
      0,
  ...
  /* --------------------------------------------------------------------*/
  phase = 11;
  PARDISO(self->L->pt, &(self->L->maxfct), &(self->L->mnum), &(self->L->mtype),
          &phase, &n, a, ja, ia, &(self->L->idum), &nrhs, self->L->iparm,
          &(self->L->msglvl), &(self->L->ddum), &(self->L->ddum),
          &(self->L->error));
  if (self->L->error != 0) {
...
abip_int pardiso_solve(spe_problem *self, abip_float *b, abip_int n) {
  ...
  PARDISO(self->L->pt, &self->L->maxfct, &self->L->mnum, &self->L->mtype,
          &phase, &n, K->x, K->p, K->i, &self->L->idum, &nrhs, self->L->iparm,
          &self->L->msglvl, b, x, &self->L->error);
  if (self->L->error != 0) {
...
abip_int pardiso_free(spe_problem *self) {
  ...
  MKL_INT n = K->n;
  PARDISO(self->L->pt, &self->L->maxfct, &self->L->mnum, &self->L->mtype,
          &phase, &n, &self->L->ddum, K->p, K->i, &self->L->idum, &nrhs,
          self->L->iparm, &self->L->msglvl, &self->L->ddum, &self->L->ddum,
          &self->L->error);
...
abip_int LDL_factor(cs *A, cs **L, abip_float *Dinv) {
  ...
  // now factor
  abip_int status =
      QDLDL_factor(A->n, A->p, A->i, A->x, (*L)->p, (*L)->i, (*L)->x, D, Dinv,
...
abip_int pcg(spe_problem *self, abip_float *b, abip_float *x, abip_float rho_x,
             abip_int max_iter, abip_float tol) {
  /*
  ...
  */
...
#if EXTRA_VERBOSE > 0
      abip_printf(
          "CG took %d iterations to converge, resisdual %4f <= tolerance %4f\n",
          i, ABIP(norm)(r, m), tol);
#endif
  ...
  printf(
      "CG did not converge within %d iterations, resisdual %4f > tolerance "
      "%4f\n",
...
abip_int qcp_pcg(spe_problem *self, abip_float *b, abip_float *x,
  ...
  */
  ...
  abip_float *p =
      (abip_float *)abip_calloc(n, sizeof(abip_float)); /* cg direction */
  abip_float *Gp = (abip_float *)abip_calloc(
      n, sizeof(abip_float)); /* updated CG direction */
  abip_float *r =
      (abip_float *)abip_calloc(n, sizeof(abip_float)); /* cg residual */
  abip_float *z = (abip_float *)abip_calloc(n, sizeof(abip_float));
  ; /* for preconditioning */
...
static void svm_mat_vec(spe_problem *self, const abip_float *x, abip_float *y) {
  ...
  for (i = 0; i < self->q; i++) {
    if (i < self->n)
      H[i] = self->stgs->rho_x + self->Q->x[i];
    else
      H[i] = self->stgs->rho_x;
  }
...
SVM
*/
abip_int svmqp_pcg(spe_problem *self, abip_float *b, abip_float *x,
                   abip_int max_iter, abip_float tol) {
  /*
      x is used for warm start
      result overwrite b
...
abip_int ABIP(solve_linsys)(spe_problem *spe, abip_float *b, abip_int n,
                            abip_float *pcg_warm_start, abip_float pcg_tol) {
  if (spe->stgs->linsys_solver == 0) {  // mkl_dss
