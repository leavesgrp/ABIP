 ...
 * for details.  If the ABIPInfo array is not present, nothing is printed.
 */
...
)
{
            abip_float n; 
            abip_float ndiv; 
            abip_float nmultsubs_ldl; 
            abip_float nmultsubs_lu;
            abip_float lnz;
            abip_float lnzd;
            ...
            n = ABIPInfo [AMD_N] ;
            ndiv = ABIPInfo [AMD_NDIV] ;
            nmultsubs_ldl = ABIPInfo [AMD_NMULTSUBS_LDL] ;
            nmultsubs_lu = ABIPInfo [AMD_NMULTSUBS_LU] ;
            lnz = ABIPInfo [AMD_LNZ] ;
            lnzd = (n >= 0 && lnz >= 0) ? (n + lnz) : (-1) ;

            /* AMD return status */
            PRINTF (("    status: ")) ;
            if (ABIPInfo [AMD_STATUS] == AMD_OK)
            {
	           PRINTF (("OK\n")) ;
            }
            else if (ABIPInfo [AMD_STATUS] == AMD_OUT_OF_MEMORY)
            {
	           PRINTF (("out of memory\n")) ;
            }
            else if (ABIPInfo [AMD_STATUS] == AMD_INVALID)
            {
	           PRINTF (("invalid matrix\n")) ;
            }
            else if (ABIPInfo [AMD_STATUS] == AMD_OK_BUT_JUMBLED)
            {
