...
(
            ...
            abip_float Control [ ],	/* array of size AMD_CONTROL */
            abip_float ABIPInfo [ ]	/* array of size AMD_INFO */
)
{
 ...
 *
 * Control:  A abip_float array of size AMD_CONTROL containing input parameters
 *	that affect how the ordering is computed.  If ABIP_NULL, then default
 *	settings are used.
 *
 ...
 *
 * ABIPInfo:  A abip_float array of size AMD_INFO.  If present, (that is, not ABIP_NULL),
 *	then statistics about the ordering are returned in the ABIPInfo array.
...
/* ========================================================================= */
            ...
            * size-n elbow room is enforced. */
            ...
            if (Control != (abip_float *) ABIP_NULL)
            {
            ...
            AMD_DEBUG1 (("\n======Nel "ID" initial\n", nel)) ;
            AMD_dump (n, Pe, Iw, Len, iwlen, pfree, Nv, Next, Last, Head, Elen, Degree, W, -1) ;
            ...
            while (nel < n)
            {
	           ...
	           if (AMD_debug >= 2)
	           {
	                       AMD_dump (n, Pe, Iw, Len, iwlen, pfree, Nv, Next, Last, Head, Elen, Degree, W, nel) ;
	           }
	           ...
	           */
	           ...
	           if (ABIPInfo != (abip_float *) ABIP_NULL)
	           {
	                       f = nvpiv ;
	                       r = degme + ndense ;
	                       dmax = MAX (dmax, f + r) ;
            ...
            if (ABIPInfo != (abip_float *) ABIP_NULL)
            {
	           ...
	           f = ndense ;
	           dmax = MAX (dmax, (abip_float) ndense) ;
	           ...
	           ABIPInfo [AMD_LNZ] = lnz ;

	           /* number of divide ops for LU and LDL' */
	           ABIPInfo [AMD_NDIV] = ndiv ;

	           /* number of multiply-subtract pairs for LDL' */
	           ABIPInfo [AMD_NMULTSUBS_LDL] = nms_ldl ;

	           /* number of multiply-subtract pairs for LU */
	           ABIPInfo [AMD_NMULTSUBS_LU] = nms_lu ;

	           /* number of "dense" rows/columns */
	           ABIPInfo [AMD_NDENSE] = ndense ;

	           /* largest front is dmax-by-dmax */
	           ABIPInfo [AMD_DMAX] = dmax ;

	           /* number of garbage collections in AMD */
	           ABIPInfo [AMD_NCMPA] = ncmpa ;
