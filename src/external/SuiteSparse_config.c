...
struct SuiteSparse_config_struct SuiteSparse_config =
{
            ...
            /* memory management functions */
                        ...
                        ABIP_NULL, ABIP_NULL, ABIP_NULL, ABIP_NULL,
            #endif
...
/* All applications that use SuiteSparse should call SuiteSparse_start prior
   ...
   function pointers to ABIP_NULL (which tells SuiteSparse to use the default
...
void SuiteSparse_start ( void )
{
            ...
            #else
                        /* no memory manager defined; you must define one after calling SuiteSparse_start */
                        SuiteSparse_config.malloc_func  = ABIP_NULL ;
                        SuiteSparse_config.calloc_func  = ABIP_NULL ;
                        SuiteSparse_config.realloc_func = ABIP_NULL ;
                        SuiteSparse_config.free_func = ABIP_NULL ;
            #endif
            ...
            #else
                        /* printf is disabled */
                        SuiteSparse_config.printf_func = ABIP_NULL ;
...
)
{
            ...
            if (size != ((abip_float) nitems) * size_of_item)
            {
                        /* size_t overflow */
                        p = ABIP_NULL ;
            }
...
void *SuiteSparse_realloc   /* pointer to reallocated block of memory, or to original block if the realloc failed. */
(
...
)
{
            ...
            if (size != ((abip_float) nitems_new) * size_of_item)
            {
                        /* size_t overflow */
                        (*ok) = 0 ;
            }
            ...
            else
            {
                        ...
                        if (pnew == ABIP_NULL)
                        {
                                    ...
                                    {
                                                /* the attempt to reduce the size of the block failed, but the old block is unchanged.  So pretend to succeed. */
                                                (*ok) = 1 ;
                                    }
                                    else
                                    {
...
void *SuiteSparse_free      /* always returns ABIP_NULL */
(
 ...
 * returned.  A scalar abip_float precision value for 'tic' could be used, but this
 ...
 *
 * A abip_float array of size 2 is used so that this routine can be more easily
...
(
            abip_float tic [2]      /* output, contents undefined on input */
)
{
            /* POSIX C 1993 timer, requires -librt */
            struct timespec t ;
            clock_gettime (CLOCK_MONOTONIC, &t) ;
            tic [0] = (abip_float) (t.tv_sec) ;
            tic [1] = (abip_float) (t.tv_nsec) ;
}
...
abip_float SuiteSparse_toc  /* returns time in seconds since last tic */
(
            abip_float tic [2]  /* input, not modified from last call to SuiteSparse_tic */
)
{
            abip_float toc [2] ;
            SuiteSparse_tic (toc) ;
...
abip_float SuiteSparse_time  /* returns current wall clock time in seconds */
(
            void
)
{
 ...
 * The NaN cases for the abip_float relops x >= y and x+y == x are safely ignored.
 * 
 ...
 * for the abip_float relop br >= bi.  Returns 1 if the denominator is zero,
 * 0 otherwise.
 *
...
(
            abip_float ar, 
            abip_float ai,       /* real and imaginary parts of a */
            abip_float br, 
            abip_float bi,       /* real and imaginary parts of b */
            abip_float *cr, 
            abip_float *ci      /* real and imaginary parts of c */
)
{
            abip_float tr; 
            abip_float ti; 
