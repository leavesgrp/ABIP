 ...
 * The int array Ai and the abip_float array Ax are of size nz.  This data structure
 ...
 * abip_float array Lx of the same size as Li).  It has a unit diagonal, which is
...
(
    	...
    	LDL_int Ap [ ],		/* input of size n+1, not modified */
    	LDL_int Ai [ ],		/* input of size nz=Ap[n], not modified */
    	abip_float Ax [ ],	/* input of size nz=Ap[n], not modified */
    	...
    	abip_float Lx [ ],	/* output of size lnz=Lp[n], not defined on input */
    	abip_float D [ ],		/* output of size n, not defined on input */
    	abip_float Y [ ],		/* workspace of size n, not defn. on input or output */
...
)
{
		...
		{
        			abip_printf("Interrupt detected in factorization! \n");
        			return -1;
    		}
...
(
    	...
    	abip_float X [ ],		/* size n.  right-hand-side on input, soln. on output */
    	LDL_int Lp [ ],		/* input of size n+1, not modified */
    	LDL_int Li [ ],		/* input of size lnz=Lp[n], not modified */
    	abip_float Lx [ ]		/* input of size lnz=Lp[n], not modified */
)
{
    	LDL_int j; 
    	LDL_int p; 
...
(
    	...
    	abip_float X [ ],		/* size n.  right-hand-side on input, soln. on output */
    	abip_float D [ ]		/* input of size n, not modified */
)
{
    	LDL_int j ;
    	for (j = 0 ; j < n - 7 ; ++j)
...
(
    	...
    	abip_float X [ ],		/* output of size n. */
    	abip_float B [ ],		/* input of size n. */
...
)
{
    	...
    	if (!P)
    	{
		return (1) ;	    /* If ABIP_NULL, P is assumed to be the identity perm. */
    	}
