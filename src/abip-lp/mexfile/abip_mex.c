...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) 
{
      ...
      tmp = mxGetField(settings, 0, "dynamic_sigma");
      if (tmp != ABIP_NULL) {
            d->stgs->dynamic_sigma = (abip_float) *mxGetPr(tmp);
      }

      tmp = mxGetField(settings, 0, "dynamic_x");
      if (tmp != ABIP_NULL) {
            d->stgs->dynamic_x = (abip_float) *mxGetPr(tmp);
      }

      tmp = mxGetField(settings, 0, "dynamic_eta");
      if (tmp != ABIP_NULL) {
            d->stgs->dynamic_eta = (abip_float) *mxGetPr(tmp);
      }

      tmp = mxGetField(settings, 0, "restart_thresh");
      if (tmp != ABIP_NULL) {
            d->stgs->restart_thresh = (abip_int) *mxGetPr(tmp);
      }

      tmp = mxGetField(settings, 0, "restart_fre");
      if (tmp != ABIP_NULL) {
            d->stgs->restart_fre = (abip_float) *mxGetPr(tmp);
      }

      // add by Kurt. 22.05.03
      tmp = mxGetField(settings, 0, "origin_rescale");
      if (tmp != ABIP_NULL) {
            d->stgs->origin_rescale = (abip_int) *mxGetPr(tmp);
      }

      tmp = mxGetField(settings, 0, "pc_ruiz_rescale");
      if (tmp != ABIP_NULL) {
            d->stgs->pc_ruiz_rescale = (abip_int) *mxGetPr(tmp);
      }

      tmp = mxGetField(settings, 0, "qp_rescale");
      if (tmp != ABIP_NULL) {
            d->stgs->qp_rescale = (abip_int) *mxGetPr(tmp);
      }

      tmp = mxGetField(settings, 0, "ruiz_iter");
      if (tmp != ABIP_NULL) {
            d->stgs->ruiz_iter = (abip_int) *mxGetPr(tmp);
      }

      tmp = mxGetField(settings, 0, "hybrid_mu");
      if (tmp != ABIP_NULL) {
            d->stgs->hybrid_mu = (abip_int) *mxGetPr(tmp);
      }

      tmp = mxGetField(settings, 0, "half_update");
      if (tmp != ABIP_NULL) {
            d->stgs->half_update = (abip_int) *mxGetPr(tmp);
      }

      tmp = mxGetField(settings, 0, "avg_criterion");
      if (tmp != ABIP_NULL) {
            d->stgs->avg_criterion = (abip_int) *mxGetPr(tmp);
      }

      tmp = mxGetField(settings, 0, "hybrid_thresh");
      if (tmp != ABIP_NULL) {
            d->stgs->hybrid_thresh = (abip_float) *mxGetPr(tmp);
      }

      tmp = mxGetField(settings, 0, "dynamic_sigma_second");
      if (tmp != ABIP_NULL) {
            d->stgs->dynamic_sigma_second = (abip_float) *mxGetPr(tmp);
      }

      tmp = mxGetField(settings, 0, "timelimit");
      if (tmp != ABIP_NULL) {
          d->stgs->max_time = (abip_float)*mxGetPr(tmp);
      }
      else {
          d->stgs->max_time = 3600;
      }
      // ------------
      ...
      tmp = mxGetField(settings, 0, "feasopt");
      if (tmp != ABIP_NULL) 
      {
            d->stgs->pfeasopt = (abip_int)*mxGetPr(tmp);
      } else {
            d->stgs->pfeasopt = 0;
      }
