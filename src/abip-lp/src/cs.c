...
{
    abip_int p, q, j, *Cp, *Ci, n, m, *Ap, *Ai, *w ;
    abip_float *Cx, *Ax ;
    cs *C ;
    m = A->m ; n = A->n ; Ap = A->p ; Ai = A->i ; Ax = A->x ;
    C = ABIP(cs_spalloc)(n, m, Ap [n], values && Ax, 0) ;       /* allocate result */
    w = abip_calloc(m, sizeof (abip_int)) ;                      /* get workspace */
    if (!C || !w) return (cs_done (C, w, NULL, 0)) ;       /* out of memory */
    Cp = C->p ; Ci = C->i ; Cx = C->x ;
    for (p = 0 ; p < Ap [n] ; p++) w [Ai [p]]++ ;          /* row counts */
    ABIP(cs_cumsum) (Cp, w, m) ;                                 /* row pointers */
    for (j = 0 ; j < n ; j++)
    {
